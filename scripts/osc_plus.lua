...
COMMIT_ d8a79a1ca801a4b0de0c316764c52872639fc3d5
DOC: https://github.com/asgod//mpv

fork by https://github.com/hooke007/MPV_lazy/commits/2027fb8b2ec766896773c6803c9b7a33a4fc6f12/portable_config/scripts/osc_plus.lua
update with mpv 0.40 master
key binding (input.conf) eg: 
 <KEY>   script-binding osc_plus/visibility   # toggle osc_plus visibility
...
local user_opts = {
    ...
    layout = "bottombox",       -- customize |bottombox| for osc_plus except original layout
...
local function set_osc_styles()
    osc_styles = {
        ...
        bb_background  =  "{\\blur100\\bord" .. user_opts.shadowsize .. "\\1c&H000000&\\3c&H000000&}",
...
local function window_controls_enabled()
    ...
    if val == "auto" then
        return not (state.border and state.title_bar) or state.fullscreen  -- show window topbar in fullscreen
    else
        return val ~= "no"
    end
end
...
local function render_elements(master_ass)
    ...
    -- bottombox layout toggle show subtitle
...
local function window_controls(topbar)
    ...
    add_area("window-controls-title_osc_plus",
...
local function osc_init()
    ...
    ne.eventresponder["mouse_move"] = --keyframe seeking when mouse is dragged
        function (element)
            ...
            -- identical seeks
            thumbfast.pause = false -- pause thumb
            -- mp.commandv("script-message-to", "thumbfast", "clear")
            local seekto = get_slider_value(element)
            if element.state.lastseek == nil or
                element.state.lastseek ~= seekto then
                    local flags = "absolute-percent+keyframes" -- ignore --hr-seek 
                    if not user_opts.seekbarkeyframes then
                        flags = "absolute-percent+exact"       -- ignore --hr-seek 
                    end
    ...
    ne.eventresponder["mouse_leave"] =
        function (element) thumbfast.pause = true end --resume thumb
...
local function render()
    ...
    if osc_param.areas["window-controls-title_osc_plus"] then
        for _,cords in ipairs(osc_param.areas["window-controls-title_osc_plus"]) do
            if state.osc_visible then -- activate only when OSC is actually visible
                set_virt_mouse_area(cords.x1, cords.y1, cords.x2, cords.y2, "window-controls-title_osc_plus")
            end
            if state.osc_visible ~= state.windowcontrols_title then
                if state.osc_visible then
                    mp.enable_key_bindings("window-controls-title_osc_plus", "allow-vo-dragging")
                else
                    mp.disable_key_bindings("window-controls-title_osc_plus", "allow-vo-dragging")
                end
...
opt.read_options(user_opts, "osc_plus", function(changed)
    validate_user_opts()
    set_osc_styles()
